...
function switchTab(index) {
  ...
  tabs.forEach((tab, idx) => {
    if (idx === index) {
      ...
      iframes[idx].classList.remove('active');
    }
  });
}
...
function closeTab(event, index) {
  ...
  const tabToClose = document.querySelector(`.tab[data-index="${index}"]`);
  const iframeToClose = document.querySelector(`iframe[data-index="${index}"]`);
...
// Function to add a tab based on the selected option
function optionalAddTab() {
  ...
  if (selectedValue == "0") {
    ...
    // Open Markdown Compiler Tab
    addMarkdownCompilerTab();
  }
}
...
function addNewTab() {
  ...
  newTab.innerHTML = `
    <img src="https://www.google.com/s2/favicons?sz=64&domain=https://floweryt6.github.io">
    Tab ${newTabIndex + 1}
    <span class="close-btn" onclick="closeTab(event, ${newTabIndex})">&times;</span>
  `;
  newTab.onclick = () => switchTab(newTabIndex);

  const newIframe = document.createElement('iframe');
  newIframe.setAttribute('src', `https://floweryt6.github.io/Flower%20Search/`);
  newIframe.setAttribute('data-index', newTabIndex);
...
function addWelcomeTab() {
  ...
  const newIframe = document.createElement('iframe');
  newIframe.setAttribute('src', `https://floweryt6.github.io/repositorys/Flower%20Browser/welcome.html`);
  newIframe.setAttribute('data-index', newTabIndex);
...
// Add the Markdown Compiler Tab
function addMarkdownCompilerTab() {
  const tabsContainer = document.getElementById('tabs');
  const iframesContainer = document.getElementById('iframes');
  ...
  const newIframe = document.createElement('iframe');
  newIframe.setAttribute('src', `https://floweryt6.github.io/apps/Flower%20Browser/markdown%20compiler.html`);
  newIframe.setAttribute('data-index', newTabIndex);
